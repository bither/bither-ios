...
- (void)queryAddressUnspent:(NSString *)addressesStr callback:(DictResponseBlock)callback andErrorCallBack:(ErrorHandler)errorCallback {
    ...
    [self queryAddressUnspent:url requestCount:1 callback:callback andErrorCallBack:errorCallback];
}

- (void)queryAddressUnspent:(NSString *)url requestCount:(int)requestCount callback:(DictResponseBlock)callback andErrorCallBack:(ErrorHandler)errorCallback {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
            if ([self dataIsError:completedOperation]) {
                ...
                [self handleError:error requestCount:requestCount retry:^(int requestCount) {
                    [self queryAddressUnspent:url requestCount:requestCount callback:callback andErrorCallBack:errorCallback];
                } andErrorCallBack:^{
                    if (errorCallback) {
                        errorCallback(error);
                    }
            ...
            if (currentUnspentOutputCount < unspentOutputCount) {
                _offset = _offset + 100;
                [self queryAddressUnspent:[NSString stringWithFormat:BLOCKCHAIR_COM_Q_ADDRESSES_UNSPENT_URL, _addressesStr, _offset] requestCount:1 callback:callback andErrorCallBack:errorCallback];
                return;
            }
            if (callback) {
                callback(_result);
