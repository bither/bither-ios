...
- (instancetype)init {
    ...
    if (![BitherSetting isUnitTest]) {
        manager.securityPolicy.pinnedCertificates = [NSSet setWithObject:[NSData dataWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"hdm.bither.net" ofType:@"cer"]]];
    } else {
        manager.securityPolicy.pinnedCertificates = [NSSet setWithObject:[NSData dataWithContentsOfFile:[[NSBundle bundleForClass:[self class]] pathForResource:@"hdm.bither.net" ofType:@"cer"]]];
    }
...
- (void)getHDMPasswordRandomWithHDMBid:(NSString *)hdmBid callback:(IdResponseBlock)callback andErrorCallBack:(ErrorHandler)errorCallback; {
    ...
    [manager GET:url parameters:nil headers:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        if (!responseObject) {
            if (errorCallback) {
                NSError *e = [[NSError alloc] initWithDomain:ERR_API_500_DOMAIN code:-1 userInfo:nil];
                errorCallback(e);
            }
            return;
        }
        
        NSNumber *random = responseObject;
        if (callback != nil) {
            callback(random);
        }
...
- (void)createHDMAddressWithHDMBid:(NSString *)hdmBid andPassword:(NSData *)password start:(int)start end:(int)end
    ...
    [manager POST:url parameters:params headers:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSArray *pubRemotes = [self split:responseObject];
        if (callback != nil) {
            callback(pubRemotes);
        }
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
...
- (void)signatureByRemoteWithHDMBid:(NSString *)hdmBid andPassword:(NSData *)password andUnsignHash:(NSArray *)unsignHashes andIndex:(int)index
    ...
    [manager POST:url parameters:params headers:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSArray *signatures = [self split:responseObject];
        if (callback != nil) {
            callback(signatures);
        }
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
...
- (void)recoverHDMAddressWithHDMBid:(NSString *)hdmBid andPassword:(NSData *)password andSignature:(NSString *)signature
    ...
    [manager POST:url parameters:params headers:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSError *error = nil;
        NSMutableDictionary *dict = [NSMutableDictionary dictionaryWithDictionary:[NSJSONSerialization JSONObjectWithData:responseObject options:0 error:&error]];
        if (error) DLog(@"JSON Parsing Error: %@", error);
