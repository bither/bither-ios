...
static GetBTWSetting *S;

@interface GetBTWSetting ()
...
@implementation GetBTWSetting

+ (Setting *)getBTWSetting {
    if (!S) {
        S = [[GetBTWSetting alloc] init];
    }
    return S;
}

- (instancetype)init {
    self = [super initWithName:[NSString stringWithFormat:NSLocalizedString(@"get_split_coin_setting_name", nil), [SplitCoinUtil getSplitCoinName:SplitBTW]] icon:nil];
    if (self) {
        __weak GetBTWSetting *s = self;
        [self setSelectBlock:^(UIViewController *controller) {
            
            u_int32_t lastBlockHeight = [BTPeerManager instance].lastBlockHeight;
            uint64_t forkBlockHeight = [BTTx getForkBlockHeightForCoin:BTW];
